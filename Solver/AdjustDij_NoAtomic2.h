#ifndef ADJUSTDIJ_NOATOMIC2_H_
#define ADJUSTDIJ_NOATOMIC2_H_

#include <assert.h>
#include <stdlib.h>
#include "../Utils/ColorUtils.h"

namespace AdjustDij_NoAtomic2Utils {
    static const int PUSH_RETRY_TIMES = 5;
    static const int CAPACITY = 256;
}

class AdjustDij_NoAtomic2: public Solver<AdjacentMatrix> {
    char name[1024];
    const int THREAD_NUM, NUM_RELAX_EACH_ITERATION;

    Heaps *heaps;

    typedef Utils::TwoDConcurrentQueue<AdjustDij_NoAtomic2Utils::CAPACITY> TwoDConcurrentQueueType;
    ...
    void incVersion(const int &pid) {
        unsigned int maxVersion = 0;
        for (int i = 0; i < THREAD_NUM; ++i)
            if (maxVersion < workerInfo[i].version) maxVersion = workerInfo[i].version;
        workerInfo[pid].version = maxVersion + 1;
    }
            ...
            for (int i = g[src_node]; i < g[src_node + 1]; ++i) {
                ...
                if (result[dst_node] > new_dst_result) {
                    if (colors[dst_node] == pid) {
                        ...
                        push_ret = false;
                        Utils::SmallConcurrentQueue<AdjustDij_NoAtomic2Utils::CAPACITY>* dst_queue = queues->get(colors[dst_node], pid);
                        for (int tmp = 0; tmp < AdjustDij_NoAtomic2Utils::PUSH_RETRY_TIMES; ++tmp) {
        ...
        while (more) {
            switch (myState) {
                case STATE_ACTIVE:
                    ...
                    adjust(pid, matrix);
                    if (isHeapQueueEmpty(pid)) myState = STATE_READY_TO_STOP;
                    break;
                case STATE_READY_TO_STOP:
                    ...
                    for (int i = 0; i < THREAD_NUM && !someStateIsActive && isFinished; ++i) {
                        if (workerInfo[i].state == STATE_ACTIVE) someStateIsActive = true;
                        if (maxVersion < workerInfo[i].version) isFinished = false;
                    }
                    if (someStateIsActive) break;
...
public:
    AdjustDij_NoAtomic2(int thread_num, int num_relax_each_iteration) :
        ...
        sprintf(name, "Dijstra + Adjustment + NoAtomic2: THREAD_NUM=%d, NUM_RELAX_EACH_ITERATION=%d", THREAD_NUM, NUM_RELAX_EACH_ITERATION);
    }
