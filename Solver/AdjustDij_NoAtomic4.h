#ifndef ADJUSTDIJ_NOATOMIC4_H_
#define ADJUSTDIJ_NOATOMIC4_H_

#include <assert.h>
#include <stdlib.h>
#include "../Utils/ColorUtils.h"
#include "../Utils/RdtscTimer.h"
namespace AdjustDij_NoAtomic4Utils {
    static const int PUSH_RETRY_TIMES = 5;
    static const int CAPACITY = 256;
}

class AdjustDij_NoAtomic4: public Solver<AdjacentMatrix> {
    char name[1024];
    const int THREAD_NUM, NUM_RELAX_EACH_ITERATION;

    Heaps *heaps;

    typedef Utils::TwoDConcurrentQueue<AdjustDij_NoAtomic4Utils::CAPACITY> TwoDConcurrentQueueType;
        ...
        while (left > 0 && !heaps->empty(pid)) {
            ...
            for (int i = g[src_node]; i < g[src_node + 1]; ++i) {
                ...
                if (colors[dst_node] == pid && result[dst_node] > new_dst_result) {
                  ...
                  push_ret = false;
                  Utils::SmallConcurrentQueue<AdjustDij_NoAtomic4Utils::CAPACITY>* dst_queue = queues->get(colors[dst_node], pid);
    ...
    bool isUpdateQueueEmpty(const int &pid) {
        for (int i = 0; i < THREAD_NUM; ++i)
          ...
          // if (!queues->get(pid, i)->empty()) return false;
        return true;
    }
    ...
    void worker(AdjacentMatrix *matrix, int &totalScan) {
        ...
        int &myVersion = ((int*) versions)[pid];
...
public:
    AdjustDij_NoAtomic4(int thread_num, int num_relax_each_iteration) :
            THREAD_NUM(thread_num), NUM_RELAX_EACH_ITERATION(num_relax_each_iteration), heaps(NULL), queues(NULL), colors(NULL), versions(NULL) {
        sprintf(name, "Dijstra + Adjustment + NoAtomic4: THREAD_NUM=%d, NUM_RELAX_EACH_ITERATION=%d", THREAD_NUM, NUM_RELAX_EACH_ITERATION);
    }
    ...
    void compute(AdjacentMatrix &adjMatrix, Colors *colors, int source, double &prepareTimeS, double &runningTimeS, double &totalScan, double &avgScan) {
         ...
         */
        versions = (int *) malloc(sizeof(int) * THREAD_NUM);
        memset((void*) versions, 0, sizeof(int) * THREAD_NUM);
...
#pragma omp atomic
        ...
        prepareTimer.Stop();
        printf("cycleTimer = %lf, checkSum = 0x%x, checkSumNoI = 0x%x\n",
               timer.GetSecond(), adjMatrix.checkSum(), adjMatrix.checkSumNoI());
        runningTimeS = timer.GetSecond();
